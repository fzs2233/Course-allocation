...
contract ICourseAllocation {
    ...
    // 设置教师姓名
    function setTeacherName(uint256 teacherId, string calldata _name) public onlyTeacher {
        teachers[teacherId].name = _name;
    }

    // 设置教师地址
    function setTeacherAddress(uint256 teacherId, address _teacherAddress) public onlyTeacher {
        teachers[teacherId].addr = _teacherAddress;
    }

    // 设置教师已分配课程
    function addTeacherAssignedCourses(uint256 teacherId, uint256 _assignedCourses) public {
        teachers[teacherId].assignedCourses.push(_assignedCourses);
    }

    // 移除教师已分配课程
    function removeTeacherAssignedCourses(uint256 teacherId, uint256 _assignedCourses) public {
        bool found = false;
        ...
        if (found) {
            ...
            teachers[teacherId].assignedCourses[indexToRemove] =  teachers[teacherId].assignedCourses[teachers[teacherId].assignedCourses.length - 1];
            // 移除数组中的最后一个元素
    ...
    // 设置教师适合程度权重
    function setTeacherSuitabilityWeight(uint256 teacherId, uint256 _suitabilityWeight) public onlyTeacher {
        teachers[teacherId].suitabilityWeight = _suitabilityWeight;
    }

    // 设置教师价值
    function setTeacherValue(uint256 teacherId, uint256 _value) public onlyTeacher {
        teachers[teacherId].value = _value;
    }

    // 设置教师对所有课程的适合程度
    function setAllTeacherCourseSuitability(uint256 teacherId, uint256[] calldata _suitabilities) public onlyTeacher {
        require(_suitabilities.length == courseIds.length, "When setting the suitability level of courses, it is necessary to have an equal number of courses" );
        for(uint256 courseIndex = 0 ; courseIndex < courseIds.length ; courseIndex++){
            teachers[teacherId].courseSuitabilities[courseIds[courseIndex]] = _suitabilities[courseIndex];
        }
    }

    // 设置教师对课程的适合程度
    function setTeacherCourseSuitability(uint256 teacherId, uint256 courseId, uint256 _suitability) public {
        teachers[teacherId].courseSuitabilities[courseId] = _suitability;
    }

    // 设置教师对所有课程的偏好程度
    function setAllTeacherCoursePreferences(uint256 teacherId, uint256[] calldata _preferences) public onlyTeacher {
        require(_preferences.length == courseIds.length, "When setting the preference level of courses, it is necessary to have an equal number of courses" );
        for(uint256 courseIndex = 0 ; courseIndex < courseIds.length ; courseIndex++){
            teachers[teacherId].coursePreferences[courseIds[courseIndex]] = _preferences[courseIndex];
        }
    }

    // 设置教师对课程的偏好程度
    function setTeacherCoursePreferences(uint256 teacherId, uint256 courseId, uint256 _preference) public onlyTeacher {
    ...
    // 添加智能体已分配课程
    function addAgentAssignedCourses(uint256 agentId, uint256 _assignedCourses) public {
        agents[agentId].assignedCourses.push(_assignedCourses);
    }

    // 移除智能体已分配课程
    function removeAgentAssignedCourses(uint256 agentId, uint256 _assignedCourses) public {
        bool found = false;
        uint256 indexToRemove;
        ...
        if (found) {
            // 将要移除的课程位置的元素替换为数组中最后一个元素
            agents[agentId].assignedCourses[indexToRemove] = agents[agentId].assignedCourses[agents[agentId].assignedCourses.length - 1];
            // 移除数组中的最后一个元素
    ...
    // 设置智能体对课程的适合程度
    function setAgentCourseSuitability(uint256 agentId, uint256 _courseId, uint256 _suitability) public {
        agents[agentId].courseSuitabilities[_courseId] = _suitability;
    }

    // 设置智能体对课程的适合程度
    function setAllAgentCourseSuitability(uint256 agentId, uint256[] calldata _suitabilities) public {
        require(_suitabilities.length == courseIds.length, "When setting the suitability level of courses, it is necessary to have an equal number of courses" );
        for(uint256 courseIndex = 0 ; courseIndex < courseIds.length ; courseIndex++){
            agents[agentId].courseSuitabilities[courseIds[courseIndex]] = _suitabilities[courseIndex];
        }
    }
    ...
    function setAllCourseImportance(uint256[] memory _importances) public {
        require(_importances.length == courseIds.length, "must update all course suitability");
        // 更新课程适合度
        for (uint256 courseIndex = 0; courseIndex < courseIds.length; courseIndex++) {
            courses[courseIds[courseIndex]].importance = _importances[courseIndex];
        }
    }

    // 添加课程分配的教师ID
    function addCourseAssignedTeacherId(uint256 courseId, uint256 _teacherId) public {
        courses[courseId].assignedTeacherId.push(_teacherId);
    }

    // 添加课程分配的智能体ID
    function addCourseAssignedAgentId(uint256 courseId, uint256 _agentId) public {
        courses[courseId].assignedAgentId.push(_agentId);
    }

    // 移除课程已分配的老师
    function removeCourseAssignedTeacherId(uint256 courseId, uint256 _teacherId) public {
        bool found = false;
        uint256 indexToRemove;
        ...
        if (found) {
            // 将要移除的课程位置的元素替换为数组中最后一个元素
            courses[courseId].assignedTeacherId[indexToRemove] = courses[courseId].assignedTeacherId[courses[courseId].assignedTeacherId.length - 1];
            // 移除数组中的最后一个元素
    ...
    // 移除课程已分配的智能体
    function removeCourseAssignedAgentId(uint256 courseId, uint256 _agentId) public {
        bool found = false;
        uint256 indexToRemove;
        ...
        if (found) {
            // 将要移除的课程位置的元素替换为数组中最后一个元素
            courses[courseId].assignedAgentId[indexToRemove] = courses[courseId].assignedAgentId[courses[courseId].assignedAgentId.length - 1];
            // 移除数组中的最后一个元素
    ...
    // 设置课程是否适合智能体
    function setCourseIsAgentSuitable(uint256 courseId, bool _isAgentSuitable) public {
    ...
    // 获取教师对课程的适合程度
    function getTeacherSuitability(uint256 _teacherId, uint256 courseId) public view returns(uint256){
        return teachers[_teacherId].courseSuitabilities[courseId];
    }

    // 获取智能体对课程的适合程度
    function getAgentSuitability(uint256 _agentId, uint256 courseId) public view returns(uint256){
        return agents[_agentId].courseSuitabilities[courseId];
    }

    // 获取教师对课程的偏好程度
    function getPreference(uint256 _teacherId, uint256 courseId) public view returns(uint256){
        return teachers[_teacherId].coursePreferences[courseId];
    }

    // 获取老师分配的课程
    function getTeacherAssignedCourses(uint256 _teacherId) public view returns(uint256[] memory){
        return teachers[_teacherId].assignedCourses;
    }

    // 获取智能体分配的课程
    function getAgentAssignedCourses(uint256 _agentId) public view returns(uint256[] memory){
        return agents[_agentId].assignedCourses;
    }

    // 获取课程被分配给的老师
    function getCoursesAssignedTeacher(uint256 _courseId) public view returns(uint256[] memory){
        return courses[_courseId].assignedTeacherId;
    }

    // 获取课程被分配给的智能体
    function getCoursesAssignedAgent(uint256 _courseId) public view returns(uint256[] memory){

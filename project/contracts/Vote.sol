...
contract Vote is IVote {
    ...
    // 添加提案
    function createProposalMultiple(
    ...
    // 添加提案
    function createMachineProposal(
        string memory _description,
        uint256 _voteforID,
        uint256[] memory _votedIds,
        uint256[] memory _votedCounts,
        uint256 _totalVote
    ) public returns (uint256) {
        uint256 newProposalId = proposalMultipleIds.length + 1;
        ...
        for (uint256 i = 0; i < _votedIds.length; i++) {
            if (_votedIds[i] != 0) {
                proposalMultiples[newProposalId].votedIds.push(_votedIds[i]);
                proposalMultiples[newProposalId].voteIdToCount[
                    _votedIds[i]
                ] = _votedCounts[i];
            }
        }
    ...
    function getVotedIds(
        uint256 _proposalId
    ) public view override(IVote) returns (uint256[] memory, uint256) {
        return (
            proposalMultiples[_proposalId].votedIds,
            proposalMultiples[_proposalId].voteforID
        );
    }

    // 查看某个提案某个选项的票数
    function getVoteIdToCount(
        uint256 _proposalId,
        uint256 _voteId
    ) public view override(IVote) returns (uint256) {
    ...
    function voteMultiple(
        ...
        require(
            proposalMultiples[_proposalId].ifVote[voter] == false,
            unicode"您已投票"
        );
        require(
            proposalMultiples[_proposalId].voteCount <
                proposalMultiples[_proposalId].totalVote,
            unicode"投票人数已满"
        );
        ...
        for (
            uint256 i = 0;
            i < proposalMultiples[_proposalId].votedIds.length;
            i++
        ) {
    ...
    function endVoteMultiple(
        ...
        for (uint256 i = 0; i < temp.length; i++) {
            if (
                proposalMultiples[_proposalId].voteIdToCount[temp[i]] >
                maxVoteCount
            ) {
                maxVoteCount = proposalMultiples[_proposalId].voteIdToCount[
                    temp[i]
                ];
                result = temp[i];
            }
    ...
    // 创建选老师提案
    function createChooseTeacherProposal(
        string memory _description,
        uint256 _voteforID,
        uint256[] memory _votedIds,
        uint256 _totalVote
    ) public override(IVote) returns (uint256) {
        return
            createProposalMultiple(
                _description,
                _voteforID,
                _votedIds,
                _totalVote
    ...
    // 创机器投票提案
    function createMachineVoteProposal(
        string memory _description,
        uint256 _voteforID,
        uint256[] memory _votedIds,
        uint256[] memory _votedCounts
    ...
    // 创建选课程提案
    function createChooseCourseProposal(
        string memory _description,
        uint256 _voteforID,
        uint256[] memory _votedIds,
        uint256 _totalVote
    ...
    // 通过接口投票
    // 选老师投票
    function voteChooseTeacher(
        address voterAddress,
        uint256 _proposalId,
        uint256 _voteId
    ) public override(IVote) {
        voteMultiple(voterAddress, _proposalId, _voteId);
    }

    // 选课程投票
    function voteChooseCourse(
        address voterAddress,
        uint256 _proposalId,
        uint256 _voteId
    ) public override(IVote) {
    ...
    // 通过接口结束投票
    // 选老师结束投票
    function endVoteChooseTeacher(
        uint256 _proposalId
    ) public view override(IVote) returns (uint256, uint256) {
        return endVoteMultiple(_proposalId);
    }

    // 选课程结束投票
    function endVoteChooseCourse(
        uint256 _proposalId
    ) public view override(IVote) returns (uint256, uint256) {
        return endVoteMultiple(_proposalId);
    }
